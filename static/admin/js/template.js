...
function listItem (item) {
  let control = [];
  if (parseInt(item['type']) === 1) control.push('<a href="' + item['url'] + '" target="_blank">访问</a>');
  if (isPermission('code') && parseInt(item['type']) === 1) control.push('<a href="javascript:" class="code">调用代码</a>');
  ...
  return '<tr class="item' + item['id'] + '"><td' + (isPermission('delete') ? '' : ' class="none"') + '><div class="check-box"><label><input type="checkbox" name="id" value="' + item['id'] + '"></label></div></td><td>' + item['type_str'] + '</td><td>' + item['name'] + '</td><td>' + item['template'] + '</td><td>' + item['template_style_id'] + '号皮肤</td><td>' + item['is_show_search'] + '</td><td>' + item['is_show_send'] + '</td><td>' + item['captcha'] + '</td><td>' + item['is_sms_verify'] + '</td><td>' + item['is_sms_notify'] + '</td>' + (isPermission('isDefault') ? '<td class="is_default">' + (item['is_default'] ? '<span class="red">是</span>' : '<a href="javascript:" class="is_default">设为默认</a>') + '</td>' : '') + '<td>' + item['date'] + '</td>' + (control.length ? '<td>' + control.join('/') + '</td>' : '') + '</tr>';
}
