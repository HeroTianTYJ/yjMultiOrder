$(function () {
  ...
  layui.use(['form', 'date'], function () {
    ...
    // 账单权限
    layui.form.on('select(bill_permit)', function (data) {
      window.location.href = searchUrl('bill_permit=' + data.value);
    });
    // 微信绑定
...
function listItem (item) {
  ...
  return '<tr class="item' + item['id'] + '"><td' + (isPermission('delete') ? '' : ' class="none"') + '><div class="check-box"><label><input type="checkbox" name="id" value="' + item['id'] + '"></label></div></td><td>' + item['name'] + '</td><td>' + item['level_name'] + '</td>' + (isPermission('isActivation') ? '<td>' + (item['is_activation'] ? '<span class="red">是</span> | <a href="javascript:" class="is_activation">取消激活</a>' : '<span class="green">否</span> | <a href="javascript:" class="is_activation">帮他激活</a>') + '</td>' : '') + '<td>' + item['permit_group'] + '</td><td>' + item['order_permit'] + '</td><td>' + item['bill_permit'] + '</td><td>' + item['date'] + '</td>' + (isPermission('wechatOpenId') ? '<td>' + (item['wechat_open_id'] || item['wechat_union_id'] ? '<span class="red">是</span> | <a href="javascript:" class="wechat_open_id">解绑</a>' : '<span class="green">否</span>') + '</td>' : '') + (isPermission('qqOpenId') ? '<td>' + (item['qq_open_id'] ? '<span class="red">是</span> | <a href="javascript:" class="qq_open_id">解绑</a>' : '<span class="green">否</span>') + '</td>' : '') + (control.length ? '<td>' + control.join('/') + '</td>' : '') + '</tr>';
}
