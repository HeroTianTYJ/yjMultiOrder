...
class Order extends Model
{
    ...
    {
        try {
            return $this->field('id,order_id,manager_id,template_id,product_id,attr,price,count,name,tel,province,' .
                'city,county,town,address,note,email,ip,payment_id,pay_id,pay_scene,pay_date,order_state_id,' .
    ...
    {
        try {
            return $this->field('order_id,order_state_id')
                ->where('id', 'IN', Request::post('ids'))
                ->where('is_recycle', Request::controller() == 'OrderRecycle' ? 1 : 0)
                ->where($this->managerId())
                ->order(['date' => 'DESC'])
    ...
    {
        try {
            return $this->field('manager_id')->where([
                'id' => Request::post('id'),
                'is_recycle' => Request::controller() == 'OrderRecycle' ? 1 : 0
            ])->where($this->managerId())->find();
        } catch (Exception $e) {
            echo $e->getMessage();
            return [];
        }
    ...
    public function modify()
    {
        ...
        $scene = ['product_id', 'attr', 'price', 'count', 'payment_id', 'order_state_id', 'express_number'];
        $data = [
        ...
        $managerOne = (new Manager())->one($this->one2()['manager_id']);
        if ($managerOne && $managerOne['level'] == 3) {
            $data['commission'] = Request::post('commission');
            $data['is_commission'] = Request::post('is_commission');
        } else {
            $data['commission'] = $data['is_commission'] = 0;
        }
    ...
    public function modify3()
    {
        ...
        if (!(new Express())->one(Request::post('express_id'))) {
            return '您选择的快递公司不存在！';
        }
