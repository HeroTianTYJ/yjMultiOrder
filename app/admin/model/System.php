...
class System extends Model
{
    ...
    public function form()
    {
        $data = [
            ...
            'distributor_enter' => Request::post('distributor_enter'),
        ...
        if ($validate->check($data)) {
            ...
            if (Request::post('distributor_enter') != Config::get('system.distributor_enter')) {
                if (
                    !preg_match('/^\w+$/', substr(Request::post('distributor_enter'), 0, -4)) ||
                    substr(Request::post('distributor_enter'), -4) != '.php'
                ) {
                    return '分销商后台入口必须是英文、数字、下划线，且以.php结尾！';
                }
                if (is_file(ROOT_DIR . '/' . Request::post('distributor_enter'))) {
                    return '系统根目录中已存在' . Request::post('distributor_enter') . '文件，请重新指定一个分销商后台入口！';
                }
            }
            return 1;
        } else {
