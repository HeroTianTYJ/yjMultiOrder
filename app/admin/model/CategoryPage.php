...
use app\admin\validate\CategoryPage as validate;
use Exception;
...
class CategoryPage extends Model
{
    ...
    {
        try {
            return $this->field('id,name,width,left_width,bg_color')
                ->where(['id' => 1])
    ...
    {
        try {
            return $this->field('id,name,title,keyword,description,width,left_width,bg_color,copyright,code_type,' .
